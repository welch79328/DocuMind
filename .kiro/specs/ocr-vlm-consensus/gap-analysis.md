...
7. **共識對「兩個引擎犯同樣的錯」是盲的(P1,2026-08-25 新增)**

   `field_consensus.py` 的 `_all_agree` 比對的是**引擎彼此**同不同意。兩個引擎把
   `1500` 都讀成 `1600` 時它們一致,共識給高分,**沒有任何機制知道這件事**。

   這不是缺陷,是共識機制的**固有限制**:它偵測的是「來源之間的分歧」,
   而共同錯誤沒有分歧。多跑 LLM 也救不了——LLM 看到的是同一份錯的文字。

   **對任務 7(共識效度實證)的直接影響**:該任務要驗「多引擎不一致與實際錯誤
   具正相關」。即使驗出強正相關,也**只證明了共識抓得到分歧型錯誤**,
   不能推論「共識抓得到錯誤」。驗證設計必須把兩類錯誤分開統計:

   | 錯誤類型 | 共識偵測得到嗎 |
   |---|---|
   | 引擎讀出不同答案 | ✅ 是,`disagree_penalty` 會壓到 0.3 |
   | **兩引擎讀出同一個錯答案** | ❌ **否,且信心度會很高** |

   要偵測共同錯誤,唯一的辦法是引入**架構真正不同的獨立來源**(例如影像層的
   視覺語言模型)——那正是本規格原本引入 VLM 的理由之一,而該路線已於
   2026-08-25 因「暫不考慮 GPU」關閉(見 tasks.md 的 Phase 2 執行前置決策)。

   **結論:在目前組態下,共同錯誤沒有防線,且這件事規格未曾言明。**

8. **共識信心度曾被套用在 LLM 改過的值上(P1,2026-08-25 發現並已修)**

   共識算在步驟 3b(未經 LLM 的候選上),值卻可能在步驟 4 被 LLM 校正改掉,
   而 `_apply_consensus` 只讀 `field_confidences`、不讀 `agreements`。實測:

       兩引擎都讀出 1,500 → 共識 0.98
       LLM 把值改成 15,000
       修正前:最終信心度 0.92 → **放行**(門檻 0.8)

   已於 `59b2f2a` 修正:值與所有引擎原始值都不一致時,信心度壓到
   `OCR_CONSENSUS_DISAGREE_PENALTY`。比對走 `values_agree`,故 LLM 僅換表示法
   (補千分位、全形轉半形、換紀年)不會被誤判。

   **記錄此條的理由**:所需資料(`agreements[field]["engine_values"]`)**一直都在**,
   只是沒被讀。同類問題值得複查——**算出來卻沒用在決策上的訊號**還有一個:
   LLM 的觸發條件用引擎自評信心度(`postprocessor.py` 的 `confidence < 0.85`),
   不用共識。該項評估後**不修**,理由見下。

   **為什麼不讓共識不一致觸發 LLM**:引擎不一致時信心度已被壓到 0.3,
   遠低於門檻 0.8,該欄位無論如何都進人工複核。多跑 LLM 只增加成本,
   不改變結果;而真正危險的「引擎篤定但讀錯」屬第 6 條的固有限制,
   觸發條件改成什麼都救不了。

9. **瓶頸是欄位抽取,不是文字辨識(P0,2026-09-03 首次量出)**

   本規格的絕大部分工作在改善**辨識**——多引擎共識、PP-OCRv6 升級、
   ONNX 執行器、解析度調校、VLM 規劃。2026-09-03 首次取得真實準確率數據後,
   該前提被推翻。

   **量測方法**:業主提供的 4 頁**電子謄本**(網路申領)含 PDF 文字層,
   其嵌入文字即為逐字精確的 ground truth——**不需要人工標註**。
   以 `evaluation_service.character_error_rate` 對照:

   | 處理路徑 | 字元錯誤率 | 耗時 |
   |---|---|---|
   | **PDF 文字層** | **0.15%** | **0.6s** |
   | PaddleOCR(強制走 OCR) | 14.5% | 85s |
   | Tesseract(強制走 OCR) | 38.2% | 78s |

   **關鍵結果**:文字層路徑給出 **99.85% 正確**的文字之後,
   地號、建號、面積、權利範圍**仍然一個都沒抽到**,只抽到 `owner`,
   `needs_review` 仍為 `true`。

   **就算辨識做到完美,產出仍然不能用。**

   對規格的直接影響:

   - 需求 3(視覺語言引擎)的目的是改善辨識——**而辨識已非瓶頸**。
     該需求已因 2026-08-25「暫不考慮 GPU」關閉,此處是第二個獨立理由。
   - 需求 4(共識信心度)同樣作用於辨識層,對欄位抽取無幫助。
   - **任務 3.3 的基準線可以部分不靠人工標註取得**:凡有文字層的 PDF
     都自帶真值。這對 3.1(謄本標註 2/30)是重大捷徑——
     電子謄本不需要人工標註即可入基準集。

   **下一個該查的是欄位抽取為何失敗**,而非繼續優化辨識。
